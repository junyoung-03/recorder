...
function JournalListPage({ currentUser, friendId, friendMode }) {
  ...
  useEffect(() => {
    ...
    const loadJournalData = async () => {
      ...
      if (isFriendMode) {
        journalQuery = journalQuery.in('visibility', ['friends', 'public']);
      }
    ...
    loadJournalData();
  }, [targetUserId, isFriendMode, currentUser?.id]);
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-body mt-2">
              {isFriendMode ? '친구가 공유한 일기를 확인해 보세요.' : '오늘의 생각과 하루를 플래너처럼 정리해 보세요.'}
            </p>
          </div>
          {!isFriendMode && (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-warm p-5 motion-card">
            ...
            <div className="space-y-2 text-sm">
              {journals.slice(0, 5).map((journal) => (
                <a
                  key={journal.id}
                  href={isFriendMode ? `/friend/${friendId}/journal/${journal.id}` : `/journal/${journal.id}`}
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white rounded-2xl shadow-sm border border-warm p-5 motion-card">
            ...
            {journals.length > 0 ? (
              ...
              <EmptyState
                title="작성된 일기가 없습니다."
                description={isFriendMode ? '공유된 일기가 없습니다.' : '첫 번째 글을 작성해 보세요.'}
                action={isFriendMode ? null : { label: '새 글 작성', href: '/journal/new' }}
              />
            )}
          </section>
        </div>
